...
## The code for the "Nine-dash line" was from https://blog.csdn.net/nikang3148/article/details/82871006
##
...
dis = 45
projection = 'ortho'
# projection = 'longlat'  # be careful with this projection type; not debug, causing the location shifting
...
# data$radius = 2  # for `longlat` projection
data$REFfreq <- 1.0 - data$ALTfreq

# plot
p <- ggplot() +
  # background color
  geom_polygon(data = outbound[(long %in% c(-90,90) & region == "NS") | (long %in% c(-90,90) & lat %in% c(-90,90) & region == "EW")], aes(x = X, y = Y, group = group), colour = NA, fill = "#FEFEFE") +
  # world map
  ...
  geom_path(data = outbound[(long %in% c(-180,180) & region == "NS") | (long %in% c(-90,90) & lat %in% c(-90,90) & region == "EW")], aes(x = X, y = Y, group = group), linetype = "solid", colour = "black", size = .3) +
